...
const EtServiceEdit = () => {
  ...
  const { etServiceData } = location.state || {}; // Get etServiceData from the state
  const [loading, setLoading] = useState(false);

  console.log('EtReportEdit', etServiceData);
  ...
  const [user, setUser] = useState({
    ...
    orderNumber: etServiceData.orderNumber,
    refeenceNumber: etServiceData.refeenceNumber,
    searchDate: etServiceData.searchDate,
    effectiveDate: etServiceData.effectiveDate,
    propertyAdderess: etServiceData.propertyAdderess,
    state: etServiceData.state,
    country: etServiceData.country,
    parcelNumber: etServiceData.parcelNumber,
    subDivision: etServiceData.subDivision,
    lotUnit: etServiceData.lotUnit,
    block: etServiceData.block,
    sfrPudCondo: etServiceData.sfrPudCondo,
  })
  ...
  const [tablesData, setTablesData] = useState(
    etServiceData.etvestinginfo.map((item, index) => ({
  ...
  const [tablesData2, setTablesData2] = useState(etServiceData.etopenmortagedeedinfo.map((item, index) => ({
    id: index + 1,
    indicator: 0,
    data: item,
  })));
  ...
  const [tableRowsData, setTableRowsData] = useState(etServiceData.etactivejudgmentsandliens.map((item, index) => ({
    id: index + 1,
    data: item,
  })));

  const [tableTaxInstaData, setTableTaxInstaData] = useState(etServiceData.ettaxinstallment.map((item, index) => ({
  ...
  const [nameRunData, setNameRunData] = useState(etServiceData.etnameruns.map((item, index) => ({
    id: index,
    data: item,
  ...
  useEffect(() => {
    ...
    console.log("In UseEffect", etServiceData); // Debug: Check if etServiceData is available
    if (etServiceData && etServiceData.etadditionalinformation && etServiceData.etadditionalinformation.length > 0) {
      const taxInsta = etServiceData.etadditionalinformation[0];

      const taxInfo = etServiceData.etlegaldescriptioninfo[0];
      setAdditionalInformation(taxInsta.additionalInformation);
      setSlno(taxInsta.slno); // Store slno as well
      setlegaldescription({
        slno: etServiceData.etadditionalinformation[0].slno,
        legaldescription: etServiceData.etadditionalinformation[0].legaldescription
      });

      setTaxInfo({
        slno: etServiceData.ettaxinformation[0].slNo,
        assementYear: etServiceData.ettaxinformation[0].assementYear,
        setTaxYear: etServiceData.ettaxinformation[0].taxYear,
        landValue: etServiceData.ettaxinformation[0].landValue,
        buildingValue: etServiceData.ettaxinformation[0].buildingValue,
        excemption: etServiceData.ettaxinformation[0].excemption,
        totalValue: etServiceData.ettaxinformation[0].totalValue,
        notes: etServiceData.ettaxinformation[0].notes,
      });
      setassementYear(etServiceData.ettaxinformation[0].assementYear);
      setTaxYear(etServiceData.ettaxinformation[0].taxYear);

      console.log("US in Tax", etServiceData.ettaxinformation);
    }
  }, [etServiceData]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${UserService.BASE_URL}/update/etrack`, payload, {
        headers: {
                ...
                {tablesData.map(table => (
                  <div key={table.id} index>
                    ...
                    <center>
                      <table className='et-service-form-table-1' style={{ border: '2px solid black', borderCollapse: 'collapse' }}>
                        <tr className='et-service-form-table-1-rows' >
                          <th className='et-service-form-table-selftables-heading' colSpan="7"> {table.id === 0 ? table.name : `CHAIN OF TITLE ${table.id}`} </th>
                        </tr>
                        <tr className='et-service-form-table-1-rows'>
